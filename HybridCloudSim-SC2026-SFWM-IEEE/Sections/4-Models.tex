...
\[
E_{\text{job}} = \frac{P_{\text{baseline}} \cdot t_{\text{occupy}}}{N_{\text{concurrent}}},
\]
...
The execution time of the $i$-th QPU segment is computed as
\[
t^{\text{qpu}}_i = \texttt{qpu\_finish}_i^j - \texttt{qpu\_start}_i^j,
\]
and similarly, the execution time of the $i$-th CPU segment is
\[
...
$t_{\text{occupy}}$ & h & Duration which a quantum job occupies the quantum processing unit. \\
...
Per-job energy is attributed by phase. For job $J$ with phase durations $t^{q}_{i}$ and $t^{c}_{i}$ on the devices selected in iteration $i$,
\begin{equation}
\label{eq:energy}
